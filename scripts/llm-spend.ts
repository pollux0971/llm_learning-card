...
export function parseSpendArgs(_argv: string[]): SpendCliArgs {
  throw new Error('not implemented: parseSpendArgs (03-llm-router/phase-4)');
}
 ...
 * 沒有 `--json` 時印這個。
 */
export function formatSpendLine(_spentUsd: number, _calls: number, _capUsd: number): string {
  throw new Error('not implemented: formatSpendLine (03-llm-router/phase-4)');
}
