 ...
 * **已完成(2026-09-12 核對)**:`state.ts` 的四步版本已經在位(`fsyncDir` 在 rename 之後),
 * 這一整組 **82/82 綠**。
 *
 * ⚠️ 上面那句**留著劃掉、不刪除**,因為它本身是一個值得記的坑:
 * 「這一組現在是紅的」這種註解,實作補上之後**沒有任何守門會告訴你它過期了** ——
 * 測試從紅變綠是好事,而好事不會觸發警報。三個檔各留了一句(ADR-040 一句、ADR-041 兩句),
 * 全部在實作合併之後**繼續躺了好幾週**,直到協調者做維護清單第 1 項才撞見。
 * **危險的方向是它會讓下一個人把「真的紅」讀成「已知的待辦」而不去查。**
 * `check:doc-rot` 掃不到這種(它掃的是文件,不是程式碼註解裡的測試狀態宣稱)。
 * 規則:**寫「現在是紅的」的時候,連「什麼時候該把這句拿掉」一起寫**;
 * 沒有退場條件的狀態宣稱會活得比它描述的狀態久。
 */
