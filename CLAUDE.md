...
- 守門腳本採用的模板版本**以 `scripts/` 檔頭的 SOURCE 標頭為準**,不在這裡記版號(模板在 `/data/python/dev-paradigm`;`_root.ts` 與 `check-*.ts` 檔頭有來源標頭,用 `sync-gates.sh` 升版,不手改;落點表與例外清單在 `scripts/boundaries.owners.json`、`boundaries.allow.json` 與 `gates.config.json`,那三個是我們自己的設定,模板不覆蓋)
- schema:`zod`
...
(名稱會變,用 `ListAgents` 找「llm-learning-cards-」開頭且不是協調者的那個),它決定後 `/decide` 記 ADR。只有三種情況問使用者:
