...
> **這是互動用的最小集。** 派工的驗收用工單模板(`.orca-brief/TASK.md` 的「驗收」段),
> 那邊比這裡嚴:有反向驗證、逐項列前提各自實測、P-29 的破壞驗證。
> **跑完這個 skill 不等於通過派工驗收** —— 兩者強度不同,不要互相代替。
...
1. 這個 phase 做出來的東西,**現在**守著 / 處理 / 驗證的是**哪些真實對象**?列出來。
2. 有沒有哪個對象**列不出來**?(忘了填)
...
三段對應三種真實踩過的形狀:
- **掃描器掃到 0 個檔** → exit 0,「看起來很乾淨」
- **golden registry 只有 demo** → 框架完美,守著假資料
- **registry 的 key 型別** → `children.md` 跟另外兩個檔共用一個 key,**不可能**被單獨守著

第三種最難發現:前兩種是「忘了填」,第三種是「填不進去」。
...
**能列出來的東西就寫成守門**,不要靠下次有人記得問。

**還有一步**:對照表(FEATURE.md 的門檻欄、standalone.json、守門清單)**每一列都拿掉那個守門重跑一次**
—— 沒有任何東西變紅的那一列,就是「宣稱有守但實際沒守」。這比讀對照表快,而且不會被文字騙。上面那個案例的守門是:
...
**核心項全過**(以下面這張清單為準,**不要記數量** —— 清單會長,寫死的數字會過期)→ done:
1. 非 `@manual` 場景全過
