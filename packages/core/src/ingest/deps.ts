...
/**
 * TODO(未實作,本輪只有簽章與測試):移除一個分類的**過期**圖資料。
 *
 ...
 *   - `deps.json` 存在但沒有 `category` 這個 key → 其他 key 原樣保留。
 *   - `graph/order-<category>.json` 不存在 → 直接略過。
 *
 ...
 */
export function removeCategoryGraph(_outDir: string, _category: CategoryId): void {
...
export async function analyzeDependencies(
  ...
  if (cycle.hasCycle) {
    ...
    if (repaired.unresolved) {
      ...
      //
      // TODO(未實作,本輪只有測試):這裡還要 removeCategoryGraph(outDir, category)。
