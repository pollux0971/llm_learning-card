...
import type { BaselineInfo, CompareItem, CompareResult, GoldenSetId } from './types.js';
...
function makeRun(
  ...
  const dir = join(baseDir, set, date);
  mkdirSync(dir, { recursive: true });
  writeFileSync(
    join(dir, 'meta.json'),
    JSON.stringify({ set, task: 'selftest', date, model: 'claude-sonnet-5', provider: 'anthropic', promptFileGitCommit: opts.commit ?? 'aaa1111', mode: 'live' }),
  );
  ...
  writeFileSync(join(dir, 'SCORES.md'), opts.scores ?? renderScoresSheet(set, date, Object.keys(outputs)));
  return dir;
}

describe('markBaseline / findBaseline', () => {
  it('沒有基準時 findBaseline 回 undefined', () => {
    expect(findBaseline(newBaseDir(), 'selftest')).toBeUndefined();
  });

  it('標了基準之後找得回來,而且目錄裡多了一個標記檔', () => {
    const base = newBaseDir();
    const dir = makeRun(base, 'selftest', '2026-09-10', { 'demo-1': 'x' }, { commit: 'aaa1111' });

    const info = markBaseline(base, dir);
    expect(info).toEqual<BaselineInfo>({ set: 'selftest', dir, date: '2026-09-10', promptFileGitCommit: 'aaa1111' });
    expect(existsSync(join(dir, BASELINE_MARKER))).toBe(true);
    expect(findBaseline(base, 'selftest')).toEqual(info);
  });

  it('基準只立一次:再標第二次會拒絕,舊基準原封不動', () => {
    const base = newBaseDir();
    const first = makeRun(base, 'selftest', '2026-09-10', { 'demo-1': 'x' });
    const second = makeRun(base, 'selftest', '2026-09-11', { 'demo-1': 'y' });
    markBaseline(base, first);

    expect(() => markBaseline(base, second)).toThrow(BaselineAlreadyExistsError);
    expect(findBaseline(base, 'selftest')!.dir).toBe(first);
    expect(existsSync(join(second, BASELINE_MARKER))).toBe(false);
  });

  it('標記檔的內容就是那次 run 的 meta,不用另外維護索引', () => {
    const base = newBaseDir();
    const dir = makeRun(base, 'selftest', '2026-09-10', { 'demo-1': 'x' });
    markBaseline(base, dir);
    const marker = JSON.parse(readFileSync(join(dir, BASELINE_MARKER), 'utf8')) as { set: string; date: string };
    expect(marker.set).toBe('selftest');
  ...
  it('拒絕的錯誤說得出是哪個任務、舊基準在哪個目錄', () => {
    ...
    try {
      ...
      expect(err.set).toBe('selftest');
      expect(err.existingDir).toBe(first);
      expect(err.message).toContain('selftest');
  ...
  it('真的有多個標記檔時取日期最早的那個', () => {
    const base = newBaseDir();
    const late = makeRun(base, 'selftest', '2026-09-12', { 'demo-1': 'z' });
    const mid = makeRun(base, 'selftest', '2026-09-11', { 'demo-1': 'y' });
    const early = makeRun(base, 'selftest', '2026-09-10', { 'demo-1': 'x' });
    ...
    expect(findBaseline(base, 'selftest')!.dir).toBe(early);
    expect(findBaseline(base, 'selftest')!.date).toBe('2026-09-10');
  });
  ...
  it('task 目錄裡夾雜檔案時跳過,不會當成 run 目錄', () => {
    const base = newBaseDir();
    const dir = makeRun(base, 'selftest', '2026-09-10', { 'demo-1': 'x' });
    writeFileSync(join(base, 'selftest', 'AAA-筆記.md'), '這不是 run 目錄');
    markBaseline(base, dir);
    expect(findBaseline(base, 'selftest')!.dir).toBe(dir);
  });

  it('不同 golden set 的基準各自獨立', () => {
    const base = newBaseDir();
    const a = makeRun(base, 'selftest', '2026-09-10', { 'demo-1': 'x' });
    const b = makeRun(base, NOT_REGISTERED, '2026-09-10', { 'demo-1': 'y' });
    markBaseline(base, a);
    markBaseline(base, b);
    expect(findBaseline(base, 'selftest')!.dir).toBe(a);
    expect(findBaseline(base, NOT_REGISTERED)!.dir).toBe(b);
  });
});
...
describe('detectPromptDrift', () => {
  it('prompt 檔的 commit 跟基準不一樣時報漂移,並指出檔案與兩個 commit', () => {
    const base = newBaseDir();
    markBaseline(base, makeRun(base, 'selftest', '2026-09-10', { 'demo-1': 'x' }, { commit: 'aaa1111' }));

    const drift = detectPromptDrift(base, 'selftest', 'bbb2222');
    expect(drift).toEqual({
      promptFile: expect.stringContaining('selftest'),
  ...
  it('commit 一樣就沒有漂移', () => {
    const base = newBaseDir();
    markBaseline(base, makeRun(base, 'selftest', '2026-09-10', { 'demo-1': 'x' }, { commit: 'aaa1111' }));
    expect(detectPromptDrift(base, 'selftest', 'aaa1111')).toBeUndefined();
  });

  it('還沒有基準就談不上漂移', () => {
    expect(detectPromptDrift(newBaseDir(), 'selftest', 'bbb2222')).toBeUndefined();
  });
   ...
   * (`not-registered` 是永遠不會被登記的 id,正好是這個狀態。)
   */
  it('有基準但 golden set 從 registry 不見了:丟錯,不是靜靜回 undefined', () => {
    const base = newBaseDir();
    markBaseline(base, makeRun(base, NOT_REGISTERED, '2026-09-10', { 'demo-1': 'x' }, { commit: 'aaa1111' }));
    expect(() => detectPromptDrift(base, NOT_REGISTERED, 'bbb2222')).toThrow(/有基準但沒有登記/);
  });

  it('同樣的狀況下 commit 一樣時仍然回 undefined(沒漂移就不必查 registry)', () => {
    const base = newBaseDir();
    markBaseline(base, makeRun(base, NOT_REGISTERED, '2026-09-10', { 'demo-1': 'x' }, { commit: 'aaa1111' }));
    expect(detectPromptDrift(base, NOT_REGISTERED, 'aaa1111')).toBeUndefined();
  });
});
...
describe('reviewRegression', () => {
  ...
  function twoRuns(outputsB: Record<string, string>): ReturnType<typeof compareRuns> {
    const base = newBaseDir();
    const a = makeRun(base, 'selftest', '2026-09-10', { 'demo-1': '一', 'demo-2': '二', 'demo-3': '三' }, { scores: FILLED });
    const b = makeRun(base, 'selftest', '2026-09-11', outputsB);
  ...
  it('沒變但 A 根本沒填分數的,不會憑空生出一筆', () => {
    const base = newBaseDir();
    const a = makeRun(base, 'selftest', '2026-09-10', { 'demo-1': '一' });
    const b = makeRun(base, 'selftest', '2026-09-11', { 'demo-1': '一' });
    const review = reviewRegression(compareRuns(a, b));
    expect(review.unchanged).toEqual(['demo-1']);
    expect(review.carriedForward).toEqual({});
  });

  it('golden set 跟著比對結果走,兩個清單都依字典序', () => {
    const review = reviewRegression(twoRuns({ 'demo-1': '改', 'demo-2': '二', 'demo-3': '改' }));
    expect(review.set).toBe('selftest');
