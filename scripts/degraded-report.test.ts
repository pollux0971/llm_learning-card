...
describe('量尺 · 乙 · 子行程:宣稱 --full 但 ran_all 量出來是假 → 讀不到、不比基準、不印降基準的提示、退出碼 1', () => {
  ...
  it('自己起 vitest 時:加 --reporter=json --outputFile(是加不是換,default reporter 還在),raw 目錄留下兩個證據檔,紀錄數 == 要跑的數', () => {
    ...
    //   **這個逾時值不是在描述「這段程式要跑多久」,是在描述「這台機器最壞會被吃到多滿」。**
    //   前者收窄是提高靈敏度,後者收窄是把**別人的負載**變成**我的假紅**;假紅比鬆逾時貴得多,
    //   因為它會訓練人「紅了先重跑一次看看」,那是量尺塌掉的起手式。
    //
    //   **收窄的前提(有退出條件的寬鬆是工程決定,沒有的是遺跡):**
    //   等「鎖要能自證死活」那張做完、`npm test` 全套與 Stryker 不再互相排擠之後,重量一次;
    //   若屆時 p95 仍 < 2s,收到 10s。在那之前不要照空機數字砍。
    //
    //   ⚠️ 順帶記一個教訓:2026-09-05 那句絕對句(「即使單獨跑也一樣」)很可能是在**別的
    //   worktree 正在跑 Stryker** 的機器上量的,卻被寫成程式的性質。**把當時的機器狀態寫成
    //   程式的性質** —— 寫逾時 / 重試 / 等待的理由時,一定要一起記當下的機器負載
    //   (`uptime` 一行就夠),不然下一個人重現不出來,也不知道是誰錯。
    //
    //   誠實起見:**上面那三個數字我沒有記下當時的 `uptime`**(這條規則就是這次才立的,
    //   第一次量的時候我自己也沒遵守)。只能確定當時沒有任何 worker、沒有 Stryker 在跑。
    //   **有記負載的一次是這個**,同日稍晚、有兩個 agent 在跑:
    //     `load average: 11.16, 15.25, 14.06` → 這一條 **1898ms**(整檔 8.20s)。
    //   也就是說在 load ≈ 11 之下它幾乎沒變慢(1853ms → 1898ms)。
    //   **目前負載最高的一次觀測**(技術顧問在隔離簽出跑全鏈,2026-09-12):
    //     起跑 `load 1.11` → 結束 `load 21.73, 17.65, 13.70`(中途有 worker 開工),
    //     **14 步全綠,包含這一條。** 也就是 load 21 之下全鏈沒有假紅。
    //   **但三次觀測都不是「有 Stryker 併跑」那一種**,而那正是 30s 想擋的形狀 ——
    //   所以退出條件裡要求重量一次,不是形式。
